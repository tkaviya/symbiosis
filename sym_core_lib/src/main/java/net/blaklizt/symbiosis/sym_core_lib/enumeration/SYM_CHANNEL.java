...
public enum SYM_CHANNEL
{
	...
	SYM_CHANNEL(int code, String message) { this.code = code; this.message = message; }
	
	static Map<Integer, SYM_CHANNEL> enumMap;

	public static SYM_CHANNEL valueOf(int value) {

        if(enumMap == null) {

			enumMap = new HashMap<>();

			for(SYM_CHANNEL rc : SYM_CHANNEL.values()) { enumMap.put(rc.code, rc); }
		}
