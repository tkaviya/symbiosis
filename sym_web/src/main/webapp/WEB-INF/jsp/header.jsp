...
<head>
    ...
    <script type="text/javascript">
        ...
        var ircCurrentServer = '${userSession.ircCurrentServer}';
        var ircCurrentChannel = '${userSession.ircCurrentChannel}';

        if (!!window.EventSource)
        {
            //request.setAttribute("ircCurrentServer",ircCurrentServer);
            //request.setAttribute("ircCurrentChannel",ircCurrentChannel);
            var source = new EventSource("<c:url value='/getEvents' />");
            source.onmessage = function(event)
            {
                if (event.data != '')
                {
                    $('#gameMenu').append('<br/>' + '[' + new Date().toLocaleTimeString() + ']<br/>' + event.data);
                }
            };
        }
        else
        {
            alert('Your browser does not support SSE (server sent events)!\r\n' +
                  'Upgrade to an HTML5 browser for full game functionality.');
        }
        ...
        function doAjaxPost()
        {
            ...
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/sym_web/process",
